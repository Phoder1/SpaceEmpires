...
namespace Phoder1.Core.Effects
{
    ...
    public interface IOverTimeEffect<in T> : IEffect<T>
    {
        ...
        public IDisposable ApplyEffect(T affected, IModifier<float> durationModifier);
    }
    ...
    public class OverTimeEffect<TEffect, T> : IOverTimeEffect<T> where TEffect : IEffect<T>
    {
        ...
        public IDisposable ApplyEffect(T affected) => _effect.ApplyAsOvertimeEffect(affected, Duration);

        public IDisposable ApplyEffect(T affected, IModifier<float> durationModifier)
        {
            if (durationModifier == null)
                return ApplyEffect(affected);
            else
                return _effect.ApplyAsOvertimeEffect(affected, durationModifier.ApplyModifier(Duration));
        }
    }
    ...
    public static class EffectHelper
    {
        ...
        public static IDisposable ApplyEffect<T>(this IEnumerable<IOverTimeEffect<T>> effects, T affected, IModifier<float> durationModifier)
        {
            if (effects == null || !effects.Any())
                return null;
            ...
            foreach (var effect in effects)
            {
                ...
                var instance = effect.ApplyEffect(affected, durationModifier);
        ...
        public static IDisposable ApplyAsOvertimeEffect<T>(this IEffect<T> effect, T affected, float duration)
        {
            if (effect == null)
                return null;
            ...
            async void WaitForCancel()
            {
                await Task.Delay(TimeSpan.FromSeconds(duration));

                if (AsyncManager.GlobalToken.IsCancellationRequested)
                    return;
